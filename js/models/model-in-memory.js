hiddenCuisine.Employee = Backbone.Model.extend({

    initialize:function () {
        this.reports = new hiddenCuisine.ReportsCollection();
        this.reports.parent = this;
    },

    sync: function(method, model, options) {
        if (method === "read") {
            hiddenCuisine.store.findById(parseInt(this.id), function (data) {
...
hiddenCuisine.EmployeeCollection = Backbone.Collection.extend({

    model: hiddenCuisine.Employee,

    sync: function(method, model, options) {
        if (method === "read") {
            hiddenCuisine.store.findByName(options.data.name, function (data) {
                options.success(data);
            });
        }
    }
...
hiddenCuisine.ReportsCollection = Backbone.Collection.extend({

    model: hiddenCuisine.Employee,

    sync: function(method, model, options) {
        if (method === "read") {
            hiddenCuisine.store.findByManager(this.parent.id, function (data) {
                options.success(data);
            });
        }
    }
...
hiddenCuisine.MemoryStore = function (successCallback, errorCallback) {
...
hiddenCuisine.store = new hiddenCuisine.MemoryStore();